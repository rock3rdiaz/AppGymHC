...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		*@summary: Lineas agregadas para dar colores diferentes a las citas renderizadas de acuerdo al tipo: 'funcionales' o 'medicas'
		*/
		if(htmlEscape(event.title).indexOf('VF') != -1){
			className += 'cita-funcional';
		}
		else{
			className += 'cita-medica';	
		}
